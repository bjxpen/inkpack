...
the old blob becomes garbage). After any replacement/removal, run
`gc(live=repo.iter_live_content())` to reclaim dead blobs, payloads and
dictionaries, then `verify()` to confirm the surviving content is intact.
...
- `pragmas`: `{"busy_timeout_ms": 5000, "synchronous": "NORMAL"}` (the only
  knobs; unknown keys raise `ValueError`).
